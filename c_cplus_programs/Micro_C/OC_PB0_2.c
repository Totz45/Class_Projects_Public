...
uint32_t arr = 1000;	
int hiorlo = 0; /* 0 for low and 1 for high */
uint16_t low_time = 50;	 // the time of the low pulse in number of clocks 
uint16_t high_time = 100;	// the time of the high pulse in number of clocks 
uint32_t ccr=50;
...
//set OC 011 toggle 
//OC3M Output Compare 3 Mode  - bits 4 to 6 
TIM2->CCMR2&=~(0x1<<6);
TIM2->CCMR2|=(0x1<<4);
TIM2->CCMR2|=(0x1<<5);
...
NVIC_EnableIRQ(15);	
	...
	TIM2->SR&=~(1<<3);
		...
		if (hiorlo==0)
		temp = TIM2->CNT + low_time;	
		else
		temp = TIM2->CNT + high_time;		
