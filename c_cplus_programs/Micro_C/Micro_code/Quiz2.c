...
int main(void)
{
	...
	uint8_t i; /*one's digit*/
	uint8_t j; /*ten's digit*/
  uint8_t Digit10; /*store ten's digit*/
  uint8_t Digit1; /*store one's digit*/
	/* USER CODE BEGIN 1 */
	...
	while (1)
  {
	  /*Incrementing program*/
		for	(j = 0; j < 10; j++) /*Display 3 shows tenth's place*/
		{
			Digit10 = j; /*store digit*/
			
			for (i = 0; i < 10; i++) /*Display 4 shows one's place*/
			{
				Digit1 = i; /*store digit*/
				while((GPIOA->IDR &(0x1<<6))==0) /*display current digit after press*/
				{
					Write_7Seg(3, Digit10);
					Delay(1);
					Write_7Seg(4, Digit1);
					Delay(1);
				}
				Delay(20);
				if(i < 9) /*increment to new digit*/
				{
					Digit1 = i + 1;
				}else if (j < 9) /*increment to new digit if second display needs to be incremented*/
				{
					Digit10 = j + 1;
					Digit1 = 0;
				}else /*reset to 00 after 99*/
				{
					Digit10 = 0;
					Digit1 = 0;
				}
				if((GPIOA->IDR &(0x1<<6))!=0) /*check if SW3 mapped to SW5 is pressed*/
				{
					while((GPIOA->IDR &(0x1<<6))!=0) /*display new digit when pressed*/
					{
						Write_7Seg(3, Digit10);
						Delay(1);
						Write_7Seg(4, Digit1);
