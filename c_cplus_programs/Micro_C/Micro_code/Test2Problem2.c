...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1)
  {
		while((GPIOA->IDR &(0x1<<6))==0 && (GPIOA->IDR &(0x1<<5))==0) /*display current digit after press*/
		{
			Write_7Seg(1, Digit4);
			Delay(1);
			Write_7Seg(2, Digit3);
		...
		if((GPIOA->IDR &(0x1<<5))!=0) /*check if SW3 mapped to SW5 is pressed*/
		{
			Digit4 = Digit4 + 1;
			while((GPIOA->IDR &(0x1<<5))!=0) /*display new digit when pressed*/
			{
						Write_7Seg(1, Digit4);
						Delay(1);
						Write_7Seg(2, Digit3);
...
void Write_7Seg(uint8_t temp_Enable, uint8_t temp_Digit)
{
	uint8_t Enable[5] = {0x00, 0x08, 0x04, 0x02, 0x01}; /* common anode use 1 to enable a segment and zeros to light the segments */
		 uint8_t Digit[16]= {0xC0, 0xF9, 0xA4, 0xB0, 0x99, 0x92, 0x82, 0xF8, 0x80, 0x90, 0x88, 0x83, 0xC6, 0xA1, 0x86, 0x8E};
