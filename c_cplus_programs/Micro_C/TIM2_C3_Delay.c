...
TIM2->DIER&=~(1<<3);	// disable interrupt of channel 3			
...
void TIM2_C3_Delay(int delay)
{
//delay is in ms and each count takes 1 ms you do not need to convert time to counts
		
uint16_t temp=TIM2->CNT+delay;	
 if (temp>=arr)	
		 temp=temp-arr;
...
Done=0;
 ...
 while(Done==0)
		{}  // wait for interrupt after delay ms
