...
signal count_r, slam : std_logic := '0'; --reset program

BEGIN
  
  --visual display
  PROCESS(Clocking, count_r, disp_ena, serve, press1, press2, row, column, counter_u, counter_u2, green_m, red_m, blue_m, countR, countL, volleyCount, green_d, red_d, blue_d, green_b, blue_b, red_b)
	...
	if(rising_edge(Clocking)) then
		if(counter > count_max) then
			...
			if(reset_dis = '1' and count_r = '0') then
				...
				if(slam = '1' and bounceC = '1') then
					min_col := min_col - 8;
					min_row := min_row;
				elsif(slam = '1' and bounceC = '0') then
					min_col := min_col + 8;
					min_row := min_row;
				else
					if(bounceC = '1') then
