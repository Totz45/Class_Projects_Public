...
--                           (JWB Sept.2019)
-- Rev 0.7 -- TTU file is now Quartus compliant
--                           (TF Feb.2024)
--
-- ================================================
...
architecture RTL of my_AND3 is
begin
  process(x1, x2, x0) is
  begin
    y <= x1 and x2 and x0;
...
architecture RTL of my_AND4 is
begin
  process(x1, x2, x3, x0) is
  begin
...
architecture RTL of my_XOR2 is
begin
  process(x1, x0) is
  begin
    y <= x1 xor x0;
...
architecture RTL of my_XOR3 is
begin
  process(x1, x2, x0) is
  begin
    y <= x1 xor x2 xor x0;
...
architecture RTL of my_XOR4 is
begin
  process(x1, x2, x3, x0) is
  begin
...
architecture RTL of my_NOR2 is
begin
  process(x1, x0) is
  begin
    y <= x1 nor x0;
...
architecture RTL of my_NOR3 is
begin
  process(x1, x2, x0) is
  begin
...
architecture RTL of my_NOR4 is
begin
  process(x1, x2, x3, x0) is
  begin
...
architecture RTL of my_NAND2 is
begin
  process(x1, x0) is
  begin
    y <= x1 nand x0;
...
architecture RTL of my_NAND3 is
begin
  process(x1, x2, x0) is
  begin
...
architecture RTL of my_NAND4 is
begin
  process(x1, x2, x3, x0) is
  begin
...
architecture RTL of my_XNOR2 is
begin
  process(x1, x0) is
  begin
    y <= x1 xnor x0;
...
architecture RTL of my_XNOR3 is
begin
  process(x1, x2, x0) is
  begin
...
architecture RTL of my_XNOR4 is begin
  process(x1, x2, x3, x0) is
  begin
...
entity my_decoder_3to8 is
	port ( 	sel : in STD_LOGIC_VECTOR (2 downto 0);
			y : out STD_LOGIC_VECTOR (7 downto 0));
end my_decoder_3to8;

architecture behavioral of my_decoder_3to8 is
	begin
		with sel select
			y <=	"00000001" when "000",
					"00000010" when "001",
...
entity my_mux_2to1 is
	port(	A,B : in STD_LOGIC;
			S: in STD_LOGIC;
			Z: out STD_LOGIC);
end my_mux_2to1;
 
architecture behavioral of my_mux_2to1 is
	begin
...
entity my_mux_4to1 is
 port(A,B,C,D : in STD_LOGIC;
      S0,S1: in STD_LOGIC;
      Z: out STD_LOGIC);
end my_mux_4to1;
 
architecture behavioral of my_mux_4to1 is
	begin
...
architecture RTL of my_SR_LATCH is begin
  Q    <= R nor Qnot;
  Qnot <= S nor Q;
end RTL;
...
architecture RTL of my_SRFF is begin
...
architecture RTL of my_DFF is begin
  process(D,CLK,RESET)
  begin
    if(RESET='1') then		-- async reset
       Q <= '0';
...
---------------------------
--- 7 segment display excluding dp
---------------------------
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
use work.TTU.all;

entity my_sevenSegment is port(
	sel : in STD_LOGIC_VECTOR (3 downto 0);
	sevSeg: out std_logic_vector(7 downto 0));
end my_sevenSegment;

architecture RTL of my_sevenSegment is begin
	with sel select
			sevSeg <=	"11000000" when "0000",
							"11111001" when "0001",
							"10100100" when "0010",
							"10110000" when "0011",
							"10011001" when "0100",
...
architecture arch_UpDown of my_UpDownCounter is
	signal current_count : unsigned(NumBits-1 downto 0);
	signal delay_wait, delay_ms : integer;
	constant milli : integer := 1000; 
	begin
	...
	counter_reset: process (clock, reset) begin
		if (reset = '1') then
			...
			if(delay_ms > milli) then
				delay_ms <= 0;
				if(delay_wait > delay) then
					delay_wait <= 0;
					if (direction='0') then 
...
entity my_ripple_counter is
 generic (n : natural := 4);
 port ( clk : in STD_LOGIC;
        clear : in STD_LOGIC;
        dout : out STD_LOGIC_VECTOR(n-1 downto 0)
 );
end my_ripple_counter;

architecture arch_rtl of my_ripple_counter is
